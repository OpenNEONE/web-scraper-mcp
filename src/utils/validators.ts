...
export class InputValidator {
   ...
   * 验证数字输入
   */
  static validateNumber(value: number, fieldName: string, min: number = 0, max: number = Infinity): number {
   ...
   * 验证字符串输入
   */
  static validateString(value: string, fieldName: string, minLength: number = 0, maxLength: number = Infinity): string {
...
export const ValidationUtils = {
   ...
   * 批量验证输入参数
   */
  validateInputs(inputs: { [key: string]: any }, rules: { [key: string]: (value: any) => any }): { [key: string]: any } {
